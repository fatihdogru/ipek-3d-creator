// Compact 3D simplex noise (Gustavson's reference implementation, ported).
// Deterministic permutation table, so the field looks the same on every load.
...
export function snoise3(xin: number, yin: number, zin: number): number {
  ...
  if (t0 > 0) {
    const g = PERM_MOD_12[ii + PERM[jj + PERM[kk]]] * 3;
    t0 *= t0;
  ...
  if (t1 > 0) {
    const g = PERM_MOD_12[ii + i1 + PERM[jj + j1 + PERM[kk + k1]]] * 3;
    t1 *= t1;
  ...
  if (t2 > 0) {
    const g = PERM_MOD_12[ii + i2 + PERM[jj + j2 + PERM[kk + k2]]] * 3;
    t2 *= t2;
  ...
  if (t3 > 0) {
    const g = PERM_MOD_12[ii + 1 + PERM[jj + 1 + PERM[kk + 1]]] * 3;
    t3 *= t3;
